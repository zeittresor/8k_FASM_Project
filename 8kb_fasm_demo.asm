; DISCOVERY 8K / v2.6 - PALETTE DRIFT
; FASM / Win32 / software framebuffer + procedural synth
;
; Three-act audiovisual superloop: the v2.3/v2.4 composition remains the base,
; while later acts remap motifs, phase/palette behaviour and selected geometry.
; Runtime RAM is deliberately traded for visual history instead of EXE bytes.
; PrintScreen / Win+Shift+S do not terminate the demo; ESC or click exits.
...
SN      = PT*32*BLOCKS*ACTS ; exactly 368.64 seconds / three musical acts

section '.data' data readable writeable

cls db 'd8k2',0
ttl db 'D8K v2.6 PALSHIFT',0

; diatonic C-major phase increments (C2..F5).  Index arithmetic below therefore
; creates real thirds/fifths instead of the accidental wide intervals in v2.2.
nt dw 389,436,490,519,583,654,734,778,873,980,1038,1166,1308,1468,1556,1746,1960,2077,2331,2616,2937,3111,3492,3920,4153

; Three two-bar eighth-note motifs, stored as SCALE-DEGREE OFFSETS from each
; current chord root.  255 is a real rest, so phrases breathe instead of
; becoming an endless stream of equally weighted notes.
...
; 64-bar harmonic form.  Roots are diatonic scale degrees: C Am F G etc.
; It is intentionally through-composed across two acts before the loop returns.
...
; Two-bar arrangement blocks. Bits: 0 drums, 1 lead, 2 pad, 3 full/hats,
; 4 bass, 5 motif B, 6 motif C.  Each visual scene owns two blocks/four bars.
...
@@:
 ...
 call dissolve
.show:
 ; Later acts use delayed frame history on the more geometric back half.
 mov eax,[mode]
 cmp eax,16
 jb @f
 and eax,15
...
render:
 ; Rebuild phase/camera per render call so crossfades into a new act stay smooth.
 mov eax,[tms]
 shr eax,5
 mov [slow],eax
 mov eax,[mode]
 mov edx,eax
 shr edx,4
 mov [secv],edx             ; visual act 0..2
 and eax,15
 mov [tmp],eax              ; base scene 0..15
 mov eax,edx
 imul eax,00101828h          ; packed RGB bias: act0=0, act1/subtle, act2=stronger
 mov [palbias],eax
 mov ecx,edx
 shl ecx,5
 add [slow],ecx
...
; 16-frame delayed echo: ~0.53 s at the 33 ms timer, entirely in BSS/RAM.
trail:
 inc dword [frame]
 mov eax,[frame]
 and eax,15
...
; Later-act 3D Lissajous knot: one compact curve, three frequencies, perspective.
knot:
 call dark
 mov ecx,768
.k:
...
; Act-three wire cube: 12 sampled edges, Y rotation and perspective.
cube:
 call dark
 mov ecx,768
.c:
 ...
 sub edx,32
 shl edx,1
 mov [rrx],edx               ; edge parameter -64..62
 shr eax,6                   ; edge 0..11
 mov esi,eax
 and esi,1
 shl esi,7
 sub esi,64                  ; fixed axis A +/-64
 mov ebp,eax
 and ebp,2
 shl ebp,6
 sub ebp,64                  ; fixed axis B +/-64
 shr eax,2                   ; axis group
 test eax,eax
 jz .gx
 dec eax
 jz .gy
...
.p:
 ...
 sar eax,10
 mov [fy],eax
 ; z = 180 + 26*sin(v), plus a gentle breathing camera drift
 mov eax,ebp
 movsx eax,word [sintab+eax*2]
 imul eax,26
 sar eax,10
